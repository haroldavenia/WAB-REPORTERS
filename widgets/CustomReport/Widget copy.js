define(['dojo/_base/declare', 'jimu/BaseWidget', "esri/dijit/Search", "esri/symbols/PictureMarkerSymbol", "esri/tasks/locator", "dojo/i18n!esri/nls/jsapi", "esri/layers/GraphicsLayer", "esri/toolbars/draw", "esri/graphic", "esri/geometry/webMercatorUtils", "esri/IdentityManager", "dojo/_base/lang", "dojo/dom-construct", "dojo/dom-class", "dojo/dom-style", "dojo/query", "dojo/parser", "dijit/form/TextBox", "dijit/form/Select", "dijit/Tooltip", "dijit/form/CheckBox", "dijit/form/NumberTextBox", "dijit/form/MultiSelect", "dojo/_base/window", "dijit/Dialog", "dijit/registry", "esri/tasks/geoenrichment/GeoenrichmentTask", "esri/tasks/geoenrichment/RingBuffer", "esri/tasks/geoenrichment/DriveBuffer", "esri/units"], function (declare, BaseWidget, Search, PictureMarkerSymbol, Locator, esriBundle, GraphicsLayer, Draw, Graphic, webMercatorUtils, esriId, lang, domConstruct, domClass, domStyle, query, parser, TextBox, Select, Tooltip, CheckBox, NumberTextBox, MultiSelect, win, Dialog, registry, GeoenrichmentTask, RingBuffer, DriveBuffer, Units) {
    ...
    return declare([BaseWidget], {
        ...
        _showHideLayer: function _showHideLayer(b) {
            if (!this.layerResult) {
                this._searchLayer();
            }
            if (b === true) {
        ...
        _saveResult: function _saveResult(gra, attr) {
            gra.attributes = attr;
            if (!this.layerResult) {
                this._searchLayer();
            }
        ...
        _renderDynamicRow: function _renderDynamicRow() {
                ...
                row.forEach(lang.hitch(this, function (element, j) {
                    ...
                    switch (element.type.toUpperCase()) {
                        ...
                        case "CHECK":
                            ...
                            break;
                        case "MULTICHECK":
                            elementNode = this._createMultiCheck(rowDom, element, row.length, j);
                            break;
                        case "LIST":
                    ...
                    if (element.help) {
                        var helpId = registry.getUniqueId("helpcontent");
        ...
        _createTextBox: function _createTextBox(rowDom, element, totalElements, indexActualElement) {
        ...
        _createNumber: function _createNumber(rowDom, element, totalElements, indexActualElement) {
        ...
        _createSelect: function _createSelect(rowDom, element, totalElements, indexActualElement) {
        ...
        _createMultiCheck: function _createMultiCheck(rowDom, element, totalElements, indexActualElement) {
            var contendor = domConstruct.create("div", null, rowDom);
            domConstruct.place(contendor, rowDom);
        ...
        _createCheck: function _createCheck(rowDom, element, totalElements, indexActualElement) {
            var contendor = domConstruct.create("div", null, rowDom);
            var myCheckBox = new CheckBox({
                name: element.id,
                checked: element.defaultValue ? element.defaultValue : 0
        ...
        _createList: function _createList(rowDom, element, totalElements, indexActualElement) {
        ...
        _positionElement: function _positionElement(rowDom, totalElements, indexActualElement, elementNode, element) {
            if (totalElements === 1) {
        ...
        _raddLocationToMap: function _raddLocationToMap() {
        ...
        _drawEnd: function _drawEnd(evt) {
        ...
        _selectResult: function _selectResult(e) {
            var graphic = new Graphic(e.result.feature.geometry, this.picSearch);
            this.lgisGraphicLayerSearch.add(graphic);
        },

        _rclearSelection: function _rclearSelection() {
            this.lgisGraphicLayerSearch.clear();
        },

        _clickGraphicLayerSearch: function _clickGraphicLayerSearch(e) {
            this.lgisGraphicLayerSearch.remove(e.graphic);
            this.isClick = true;
        },

        _parseBufferRadii: function _parseBufferRadii(values) {
            return values.split(",").map(function (x) {
                return parseInt(x, 10);
            });
        },

        _getDataForInputs: function _getDataForInputs() {
            var objectServices = {},
                key_i;
        ...
        _addCustomData: function _addCustomData(data) {
            data.UniqueID = 1;
            return data;
        },

        _getDataForGIS: function _getDataForGIS() {
            var objectServices = {},
                key_i;
        ...
        _addCustomGisData: function _addCustomGisData(data) {
        ...
        _parseObjectData: function _parseObjectData(data) {
            console.log(data);
        ...
        _addGeoDataGIS: function _addGeoDataGIS(data) {
            var graphic = this.lgisGraphicLayerSearch.graphics[0];
            var geo = webMercatorUtils.webMercatorToGeographic(graphic.geometry);
            data.Longitude = geo.x.toString();
            data.Latitude = geo.y.toString();
        ...
        _addToken: function _addToken(data, token) {
            //data.Token = token;
            //data.apikey = this.config.apikey;
            return data;
        },

        _requiredFieldsCheck: function _requiredFieldsCheck() {
            var status = true;
            for (var i = 0; i < this.config.configFields.rows.length; i++) {
                var row = this.config.configFields.rows[i];
                for (var j = 0; j < row.length; j++) {
        ...
        _runReport: function _runReport(e) {
            if (this._requiredFieldsCheck()) {
                ...
                var studyAreaOpts = data.BufferUnits.slice(0, 9) == "esriDrive" ? new DriveBuffer({
                    radii: radii_array,
